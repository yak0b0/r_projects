...
Using tools available in RStudio, we will conduct a comprehensive exploratory data analysis to identify patterns distinguishing successful and underperforming teams, detect statistical anomalies (e.g., teams with extreme metrics), and examine trends across seasons. The insights gained may provide valuable guidance for sports analytics, team strategy optimization, or predictive modeling of tournament success.

I will be trying to find patterns, that might occur in winning/losing teams, detecting anomalies (teams that have absurdly high/low stats in certain cafeterias) and overall looking in depth at this dataset.
...
I then decided to load in all packages that I will use during this EDA. (For now just tidyverse)
...
The POSTSEASON column could be numeric, to easier allow us to determine correlations, so we will add a new column for those values.
...
Searching through the internet (mostly reddit) I came to the conclusion that a good college season win percentage is above 65%. This also corelates to our third quartile, which implies that the NCAA is a very close and high level league.
...
I have chosen the ten teams that appeared the most, also adding on their avg postseason ranking (The lower the ranking the better the programs results throught the years).
...
Let us pressume that a bad seson will be below the first quartile, so underneath (0.3871) `WIN_P`.
...
We can notice a decrease in the amount of teams that are "bad".

Let us see which team was consistently bad.
